...
export class ActiveComponent implements OnInit {
    ...
    this.appService.getActiveApplications('Y').subscribe({
      next: (res: any) => {
        if (res.status === 'Success' && res.data?.applications) {
          ...
          this.filteredList = [...this.activeApplications];
        }
        this.loading = false;
      },
      error: (err: any) => {
  ...
  filterTable(): void {
    ...
    this.filteredList = this.activeApplications.filter((row) => {
      const values = [
        row.appNo,
        row.spaceAllocNo,
        row.vcn,
  ...
  // Returns headers based on your data keys
  ...
  private columnMap: Record<string, (row: any) => any> = {
    ...
    'Re-Measurement Date': (row) => this.formatDate(row.handoverDt),
    'Release Date': (row) =>
      row.releaseDt ? this.formatDate(row.releaseDt) : '',
  ...
  downloadPDF(): void {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('Active Application List', pageWidth / 2, 25, { align: 'center' });
    ...
    doc.save('active-applications.pdf');
  }
  ...
  downloadCSV(): void {
    ...
    saveAs(blob, 'active-applications.csv');
  }
  ...
  downloadExcel(): void {
    ...
    saveAs(blob, 'active-applications.xlsx');
  }
}
