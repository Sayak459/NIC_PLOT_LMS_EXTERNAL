...
@Component({
  selector: 'app-normal-handover',
  templateUrl: './normal-handover.component.html',
  styleUrls: ['./normal-handover.component.scss'],
})
export class NormalHandoverComponent implements OnInit {
  handoverList: any[] = [];
  ...
  ngOnInit(): void {
    ...
    console.log('✅ NormalHandoverComponent initialized');
  }
  ...
  fetchHandoverList(): void {
    ...
    this.handoverService.getNormalHandoverList().subscribe({
      next: (res: any) => {
        console.log('Normal Handover data:', res);
      ...
      error: (err: any) => {
        console.error('Error fetching normal handover list:', err);
        this.loading = false;
      },
    });
  }
  ...
  onRowUpdate(row: any) {
    ...
    const target = isInsideDashboard
      ? ['/dashboard/handover/normal-handover/update', row.appNo]
      : ['/handover/normal-handover/update', row.appNo];
  ...
  downloadPDF(): void {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('Normal Handover List', pageWidth / 2, 25, { align: 'center' });
  ...
  downloadCSV(): void {
    ...
    saveAs(blob, 'normal-handover.csv');
  }
  ...
  downloadExcel(): void {
    ...
    saveAs(blob, 'normal-handover.xlsx');
  }
}
