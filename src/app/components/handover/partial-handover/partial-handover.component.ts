...
@Component({
  selector: 'app-partial-handover',
  templateUrl: './partial-handover.component.html',
  styleUrls: ['./partial-handover.component.scss'],
})
export class PartialHandoverComponent implements OnInit {
  handoverList: any[] = [];
  ...
  ngOnInit(): void {
    ...
    console.log('✅ PartialHandoverComponent initialized');
  }

  //Handover list fetch portion
  fetchHandoverList(): void {
    this.loading = true;
    this.handoverService.getPartialHandoverList().subscribe({
      next: (res: any) => {
        console.log('Partial Handover data:', res);
  ...
  onRowUpdate(row: any) {
    ...
    const target = isInsideDashboard
      ? ['/dashboard/handover/partial-handover/update', row.appNo]
      : ['/handover/partial-handover/update', row.appNo];
  ...
  downloadPDF(): void {
    ...
    doc.setFontSize(10);
    doc.setFont('helvetica', 'bold');
    doc.text('Partial Handover List', pageWidth / 2, 25, { align: 'center' });
  ...
  downloadCSV(): void {
    ...
    saveAs(blob, 'partial-handover.csv');
  }
  ...
  downloadExcel(): void {
    ...
    saveAs(blob, 'partial-handover.xlsx');
  }
}
