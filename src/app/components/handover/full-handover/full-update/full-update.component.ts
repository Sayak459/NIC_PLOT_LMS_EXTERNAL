...
@Component({
  selector: 'app-full-update',
  templateUrl: './full-update.component.html',
  styleUrls: ['./full-update.component.scss'],
})
export class FullUpdateComponent implements OnInit {
  appNo: string = '';
  details: any = null;
  loading = true;

  form = {
    requestedFullHandoverDate: '',
    remarks: '',
    docDescription: '',
  ...
  constructor(
    ...
    private fullservice: HandoverService,
  ...
  loadDetails() {
    ...
    this.fullservice.getFullHandoverList().subscribe({
      next: (res: any) => {
  ...
  this.missingFields = [];
    ...
    if (!this.form.requestedFullHandoverDate)
  ...
  const payloadJson = {
    wla: {
      ...
      req_dt: this.form.requestedFullHandoverDate,
  ...
  // Hit API
  // ------------------------------
  this.fullservice.saveFullHandover(formData).subscribe({
    next: (res: any) => {
        // success modal
  ...
  // this.modalData = {
  //     status: 'Success',
  //     msg: 'Full Handover successfully',
  //     app_no: 'APX25XXXXXX',
  //   };
...
redirectBack() {
    ...
    if (this.router.url.includes('/dashboard')) {
      ...
      this.router.navigate(['/dashboard/handover/full-handover'], { relativeTo: this.route });
    } else {
      // Standalone route → fallback to absolute route
      this.router.navigate(['/handover/full-handover']);
    }
  }
