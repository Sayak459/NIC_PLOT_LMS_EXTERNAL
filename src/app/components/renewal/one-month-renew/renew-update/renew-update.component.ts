...
@Component({
  selector: 'app-renew-update',
  templateUrl: './renew-update.component.html',
  styleUrls: ['./renew-update.component.scss']
})
export class RenewUpdateComponent implements OnInit {
  ...
  form = {
    requestedRemeasurementDate: '', // Updated name to match remeasurement
    remarks: '',
    docDescription: '',
  ...
  loadDetails() {
    ...
    this.renewalService.getPendingRemeasurements()
      .subscribe({
        next: (res: any) => {
          ...
          if (!this.details) {
            ...
            this.form.requestedRemeasurementDate = this.details.req_dt || '';
  ...
  saveUpdate() {
    ...
    if (!this.form.requestedRemeasurementDate)
    ...
    const payloadJson = {
      wla: {
        ...
        req_dt: this.form.requestedRemeasurementDate,
        ag_remarks: this.form.remarks,
        app_no: this.appNo,
        reqNo:this.details?.reqNo,
        reqGrantYn:this.details?.reqGrantYn,
      },
    ...
    this.renewalService.saveRemeasurement(formData).subscribe({
      next: (res: any) => {
        // success modal
        ...
        //this.router.navigate(['/dashboard/renewal/one-month-renew']);
      },
      error: (err) => {
    ...
    // this.modalData = {
    //   status: 'Success',
    //   msg: 'Renewal Request successful',
    //   app_no: 'APX25XXXXXX',
    // };
  ...
  redirectBack() {
    ...
    if (this.router.url.includes('/dashboard')) {
      ...
      this.router.navigate(['/dashboard/renewal/one-month-renew'], { relativeTo: this.route });
    } else {
      // Standalone route → fallback to absolute route
      this.router.navigate(['/renewal/one-month-renew']);
    }
  }
  ...
  const target = isInsideDashboard
    ? ['/dashboard/renewal/one-month-renew']
    : ['/renewal/one-month-renew'];
